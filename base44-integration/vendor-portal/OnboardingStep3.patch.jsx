...
function CitiesSelector({ selectedCities, selectedDistricts, onCitiesChange, onDistrictsChange }) {
  ...
  const toggleCity = (city) => {
    if (selectedCities.includes(city)) {
      ...
      onDistrictsChange({ ...selectedDistricts, [city]: [] });
    }
  };
  ...
  const toggleDistrict = (city, district) => {
    ...
    const updated = current.includes(district)
      ...
      : [...current, district];
    onDistrictsChange({ ...selectedDistricts, [city]: updated });
  };
...
export default function OnboardingStep3({ data, onChange }) {
  const [selectedCities, setSelectedCities] = useState(
    () => parseJSON(data.cities_served) || []
  );
  const [selectedDistricts, setSelectedDistricts] = useState(
    () => parseJSON(data.districts_served) || {}
  );
  const [selectedEquipment, setSelectedEquipment] = useState(
    () => parseJSON(data.equipment_list) || []
  );

  useEffect(() => {
    setSelectedCities(parseJSON(data.cities_served) || []);
    setSelectedDistricts(parseJSON(data.districts_served) || {});
    setSelectedEquipment(parseJSON(data.equipment_list) || []);
  ...
  const handleCitiesChange = (cities) => {
    ...
    onChange({ ...data, cities_served: JSON.stringify(cities) });
  };

  const handleDistrictsChange = (districts) => {
    setSelectedDistricts(districts);
    onChange({ ...data, districts_served: JSON.stringify(districts) });
  };

  const handleEquipmentChange = (items) => {
    setSelectedEquipment(items);
    onChange({ ...data, equipment_list: JSON.stringify(items) });
  };
